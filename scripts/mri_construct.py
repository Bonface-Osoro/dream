...
df = pd.read_csv(os.path.join(output_years_folder, 'ZWE_combined.csv'))
...
df.to_csv(os.path.join(output_years_folder, 
                       'ZWE_spatio_temporal_malaria_indices_normalized.csv'), index = False)
...
df = df[df['year'] >= 2015]
output_path = os.path.join(DATA_RESULTS, 'mri', 'ZWE_malaria_risk_index.csv')
os.makedirs(os.path.dirname(output_path), exist_ok = True)
df.to_csv(output_path, index = False)'''

input_mri_file = os.path.join(DATA_PROCESSED, 'zimbabwe', 'combined_data', 'ZWE_ecological_predictors_with_mri.csv')
monthly_mri_output = os.path.join(DATA_RESULTS, 'mri', 'zimbabwe', 'ZWE_malaria_risk_index_monthly.csv')
